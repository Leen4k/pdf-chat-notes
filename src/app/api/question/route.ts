...
import { sendMessageToLLM, getCurrentConfig } from "@/lib/llm/index";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const results = await findSimilarChunks(questionQuery, chatId, userId);

    // Modified prompt to match second example's structure
    ...
    try {
      const rawResponse = await sendMessageToLLM(prompt);
