import { createChatSession, updateChatModel, getCurrentModel, chatSession } from "./llama-model";
...
(async () => {
  try {
    ...
    const message = "Hello, how can LlamaAI assist me today?";
    const response = await sendMessage(message);

    console.log("AI Response:", response);

    // Switch to a different model if needed
    switchModel("llama-advanced");
